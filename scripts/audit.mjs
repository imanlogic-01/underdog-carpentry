...
let nonce=0;
const request=(data=valid,headers={})=>new Request(base+"/api/enquiry",{method:"POST",headers:{"Content-Type":"application/json",Origin:base,"Idempotency-Key":crypto.randomUUID(),"cf-connecting-ip":"mock-"+(++nonce),...headers},body:JSON.stringify(data)});
...
for(let i=0;i<6;i++){assert.equal((await handleEnquiry(request(valid,{"cf-connecting-ip":"rate-limit-test"}),env,success)).status,i<5?200:429)}
